...
def _make_full_attention_kv_cache_manager(
    *,
    block_size: int = 4,
    num_blocks: int = 8,
) -> KVCacheManager:
...
def test_alias_request_blocks_shares_refcounts() -> None:
    block_size = 4
    manager = _make_full_attention_kv_cache_manager(block_size=block_size)
    src = make_request("src", list(range(8)), block_size, sha256)
    dst = make_request("dst", list(range(100, 108)), block_size, sha256)

    src_new_blocks = manager.allocate_slots(src, num_new_tokens=8)
    dst_new_blocks = manager.allocate_slots(dst, num_new_tokens=8)
    assert src_new_blocks is not None
    assert dst_new_blocks is not None
    src_blocks = list(manager.get_blocks("src").blocks[0])
    old_dst_blocks = list(manager.get_blocks("dst").blocks[0])
    assert [block.ref_cnt for block in src_blocks] == [1, 1]
    assert [block.ref_cnt for block in old_dst_blocks] == [1, 1]

    manager.alias_request_blocks(
        dst_request_id="dst",
        src_request_id="src",
        num_prefix_blocks=2,
    )

    assert manager.get_block_ids("dst") == manager.get_block_ids("src")
    assert [block.ref_cnt for block in src_blocks] == [2, 2]
    assert [block.ref_cnt for block in old_dst_blocks] == [0, 0]
    single_manager = manager.coordinator.single_type_managers[0]
    assert single_manager.num_cached_block["dst"] == 2

    manager.free(src)
    assert [block.ref_cnt for block in src_blocks] == [1, 1]
    manager.free(dst)
    assert [block.ref_cnt for block in src_blocks] == [0, 0]


def test_alias_request_blocks_truncates_self_tail() -> None:
    block_size = 4
    manager = _make_full_attention_kv_cache_manager(block_size=block_size)
    request = make_request("req", list(range(12)), block_size, sha256)
    new_blocks = manager.allocate_slots(request, num_new_tokens=12)
    assert new_blocks is not None
    blocks = list(manager.get_blocks("req").blocks[0])
    ...
    assert manager.get_block_ids("req") == ([blocks[0].block_id, blocks[1].block_id],)
    assert [block.ref_cnt for block in blocks] == [1, 1, 0]
    single_manager = manager.coordinator.single_type_managers[0]
    assert single_manager.num_cached_block["req"] == 2

    manager.free(request)
    assert [block.ref_cnt for block in blocks] == [0, 0, 0]


def test_alias_request_blocks_from_snapshot_allows_simultaneous_swap() -> None:
    block_size = 4
    manager = _make_full_attention_kv_cache_manager(block_size=block_size)
    req0 = make_request("req0", list(range(8)), block_size, sha256)
    req1 = make_request("req1", list(range(100, 108)), block_size, sha256)
    assert manager.allocate_slots(req0, num_new_tokens=8) is not None
    assert manager.allocate_slots(req1, num_new_tokens=8) is not None

    req0_blocks = manager.get_blocks("req0")
    req1_blocks = manager.get_blocks("req1")
    req0_cached = manager.get_num_cached_blocks("req0")
    req1_cached = manager.get_num_cached_blocks("req1")
    req0_block_ids = req0_blocks.get_block_ids()
    req1_block_ids = req1_blocks.get_block_ids()

    manager.alias_request_blocks_from_snapshot(
        dst_request_id="req0",
        src_blocks=req1_blocks,
        src_num_cached_blocks=req1_cached,
        num_prefix_blocks=2,
    )
    manager.alias_request_blocks_from_snapshot(
        dst_request_id="req1",
        src_blocks=req0_blocks,
        src_num_cached_blocks=req0_cached,
        num_prefix_blocks=2,
    )

    assert manager.get_block_ids("req0") == req1_block_ids
    assert manager.get_block_ids("req1") == req0_block_ids
    for block in req0_blocks.blocks[0] + req1_blocks.blocks[0]:
        assert block.ref_cnt == 1
...
def test_alias_request_blocks_repeated_lifecycle_frees_all_blocks() -> None:
    block_size = 4
    manager = _make_full_attention_kv_cache_manager(
        block_size=block_size,
        num_blocks=24,
    )
    ...
    for request in requests:
        assert manager.allocate_slots(request, num_new_tokens=12) is not None
    assert manager.block_pool.get_num_free_blocks() == initial_free_blocks - 12

    for _ in range(4):
        snapshots = {
            request.request_id: (
                manager.get_blocks(request.request_id),
                manager.get_num_cached_blocks(request.request_id),
            )
            for request in requests
        }
        for idx, request in enumerate(requests):
            src_request = requests[(idx + 1) % len(requests)]
            src_blocks, src_num_cached_blocks = snapshots[src_request.request_id]
            manager.alias_request_blocks_from_snapshot(
                dst_request_id=request.request_id,
        ...
        for request in requests:
            assert len(manager.get_blocks(request.request_id).blocks[0]) == 2
            assert manager.get_num_cached_blocks(request.request_id) == [2]
        assert manager.block_pool.get_num_free_blocks() == initial_free_blocks - 8
        live_blocks = {
            block.block_id: block
            for request in requests
            for block in manager.get_blocks(request.request_id).blocks[0]
        }.values()
        assert sum(block.ref_cnt for block in live_blocks) == 8

    for request in requests:
        manager.free(request)

    assert manager.block_pool.get_num_free_blocks() == initial_free_blocks
    assert not manager.coordinator.single_type_managers[0].req_to_blocks

...
class InputBatch:
    def __init__(
        ...
        self.power_smc_alpha = torch.zeros(
            (max_num_reqs,), dtype=torch.float32, device=device
        )
        self.power_smc_alpha_cpu_tensor = torch.zeros(
            (max_num_reqs,), dtype=torch.float32, device="cpu", pin_memory=pin_memory
        )
        self.power_smc_alpha_cpu = self.power_smc_alpha_cpu_tensor.numpy()
        self.power_smc_configs: dict[str, PowerSMCConfig] = {}
    ...
    def add_request(
        ...
        if sampling_params := request.sampling_params:
            ...
            if power_smc_config := PowerSMCConfig.from_sampling_params(
                ...
                self.temperature_cpu[req_index] = proposal_temperature(alpha_t)
    ...
    def remove_request(self, req_id: str) -> int | None:
        ...
        self.power_smc_configs.pop(req_id, None)
    ...
    def swap_states(self, i1: int, i2: int) -> None:
        ...
        self.power_smc_alpha_cpu[i1], self.power_smc_alpha_cpu[i2] = (
            self.power_smc_alpha_cpu[i2],
            self.power_smc_alpha_cpu[i1],
        )
    ...
    def condense(self) -> None:
        ...
        while empty_req_indices:
            ...
            self.power_smc_alpha_cpu[empty_index] = self.power_smc_alpha_cpu[
                last_req_index
            ]
    ...
    def refresh_metadata(self):
        ...
        if batch_update:
            ...
            self.update_power_smc_sampling_state()

    def update_power_smc_sampling_state(self) -> None:
        if not self.power_smc_configs:
            return
        if self.sampling_metadata.power_smc_alpha is None:
            self.sampling_metadata = self._make_sampling_metadata()
            return
        self._refresh_power_smc_cpu_state()
        num_reqs = self.num_reqs
        if not self.all_greedy:
            copy_slice(self.temperature_cpu_tensor, self.temperature, num_reqs)
        copy_slice(self.power_smc_alpha_cpu_tensor, self.power_smc_alpha, num_reqs)
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        power_smc_alpha = None
        if self.power_smc_configs:
            copy_slice(
                self.power_smc_alpha_cpu_tensor,
                self.power_smc_alpha,
                num_reqs,
            )
            power_smc_alpha = self.power_smc_alpha[:num_reqs]
        ...
        return SamplingMetadata(
            ...
            power_smc_alpha=power_smc_alpha,
        )

    def _refresh_power_smc_cpu_state(self) -> None:
        if not self.power_smc_configs:
            return
        for req_id, config in self.power_smc_configs.items():
            req_index = self.req_id_to_index.get(req_id)
            if req_index is None:
                continue
            alpha_t = self._power_smc_alpha_for_req_index(req_index, config)
            self.power_smc_alpha_cpu[req_index] = alpha_t
            self.temperature_cpu[req_index] = proposal_temperature(alpha_t)

    def _power_smc_alpha_for_req_index(
        self,
        req_index: int,
        config: PowerSMCConfig | None = None,
    ) -> float:
        req_id = self._req_ids[req_index]
        assert req_id is not None
        if config is None:
            config = self.power_smc_configs[req_id]
        output_token_ids = self.req_output_token_ids[req_index]
        assert output_token_ids is not None
        step = sum(token_id != -1 for token_id in output_token_ids)
        return alpha_ramp(step, config.alpha, config.alpha_ramp_tokens)

...
class Request:
    def __init__(
        ...
        self.power_smc_config = PowerSMCConfig.from_sampling_params(sampling_params)
        self.power_smc_child_info: PowerSMCChildRequestInfo | None = (
            parse_power_smc_child_request_id(request_id)
            if self.power_smc_config is not None else None
        )
    ...
    def reset_output_token_ids(self, token_ids: list[int]) -> None:
        """Replace generated tokens after scheduler-side resampling."""

        self._output_token_ids = list(token_ids)
        self._all_token_ids = (
            self.prompt_token_ids.copy()
            if self.prompt_token_ids is not None
            else [0] * self.num_prompt_tokens
        )
        self._all_token_ids.extend(self._output_token_ids)
        self.output_token_ids = ConstantList(self._output_token_ids)
        self.all_token_ids = ConstantList(self._all_token_ids)
        self.block_hashes = []
        self.update_block_hashes()

    def has_output_token_ids(self, token_ids: list[int]) -> bool:
        """Return whether generated tokens match without copying the view."""

        return self._output_token_ids == token_ids

...
from vllm.v1.outputs import LogprobsTensors, PowerSMCLogprobTensors, SamplerOutput
...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        power_smc_logits: torch.Tensor | None = None
        if sampling_metadata.power_smc_alpha is not None:
            # Keep an immutable FP32 snapshot of the base-model logits before
            # penalties, temperature, and top-k/top-p. Power-SMC validation
            # currently rejects those processors, but this keeps the sampler
            # contract explicit and independent from later in-place edits.
            power_smc_logits = logits.to(torch.float32).clone()
        ...
        if power_smc_logits is not None:
            assert sampling_metadata.power_smc_alpha is not None
            power_smc_logprobs = self.gather_power_smc_logprobs(
                power_smc_logits,
                sampling_metadata.power_smc_alpha,
                sampled,
            )
    ...
    def gather_power_smc_logprobs(
        ...
        """Gather base/proposal sampled-token logprobs for Power-SMC.

        The proposal is the full-softmax power-temperature distribution:
        ``q_t(token) = softmax(alpha_t * base_logits)[token]``.
        """

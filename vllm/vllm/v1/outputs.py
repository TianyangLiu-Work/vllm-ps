...
class PowerSMCLogprobTensors(NamedTuple):
    # [num_reqs]
    # log p_theta(sampled_token | prefix), before proposal temperature.
    base_logprobs: torch.Tensor
    # [num_reqs]
    # log q(sampled_token | prefix), after power-temperature proposal.
    proposal_logprobs: torch.Tensor

    def to_cpu_nonblocking(self) -> "PowerSMCLogprobTensors":
        if self.base_logprobs.device.type == "cpu":
            return self
        return PowerSMCLogprobTensors(
            self.base_logprobs.to("cpu", non_blocking=True),
            self.proposal_logprobs.to("cpu", non_blocking=True),
        )

    def tolists(self) -> "PowerSMCLogprobLists":
        return PowerSMCLogprobLists(
            self.base_logprobs.cpu().numpy(),
            self.proposal_logprobs.cpu().numpy(),
        )


class PowerSMCLogprobLists(NamedTuple):
    # [num_reqs]
    base_logprobs: np.ndarray
    # [num_reqs]
    proposal_logprobs: np.ndarray

    def slice_request(self, req_idx: int) -> tuple[float, float]:
        return (
            float(self.base_logprobs[req_idx]),
            float(self.proposal_logprobs[req_idx]),
        )
...
class ModelRunnerOutput:
    ...
    # Sampled-token base/proposal logprobs for Power-SMC requests.
    power_smc_logprobs: PowerSMCLogprobLists | None = None

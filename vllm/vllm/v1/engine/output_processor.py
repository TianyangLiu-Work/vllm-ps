...
class RequestState:
    ...
    def make_request_output(
        ...
        if self.parent_req is None:
            ...
            if not outputs:
                return None
            self.parent_req.rewrite_power_smc_outputs(
                outputs,
                self.detokenizer.decode_output_token_ids,
            )
    ...
    def _new_request_output(
        ...
        return RequestOutput(
            ...
            metrics=self.stats,
            power_smc=(
                self.parent_req.get_power_smc_diagnostics()
...
class OutputProcessor:
    ...
    def abort_requests(self, request_ids: Iterable[str], internal: bool) -> list[str]:
        ...
        """
        internal_req_ids = []
        parent_req_ids_to_abort: set[str] = set()
        for request_id in request_ids:
            if internal:
                ...
                parent_req_ids_to_abort.update(
                    parent_id
                    for parent_id, parent in self.parent_requests.items()
                    if parent.external_req_id == request_id
                )
        ...
        for request_id in internal_req_ids:
            ...
            elif request_id in self.parent_requests:
                parent_req_ids_to_abort.add(request_id)

        for request_id in parent_req_ids_to_abort:
    ...
    def process_outputs(
        ...
        request_outputs: list[RequestOutput | PoolingRequestOutput] = []
        reqs_to_abort: list[str] = []
        power_smc_parent_reqs: list[ParentRequest] = []
        power_smc_parent_req_set: set[ParentRequest] = set()
        power_smc_kv_events: list[tuple[ParentRequest, dict[str, Any]]] = []
        for engine_core_output in engine_core_outputs:
            ...
            if pooling_output is None:
                ...
                if req_state.parent_req is not None:
                    should_check_power_smc_resample = (
                        req_state.parent_req.observe_power_smc_step(
                            req_id,
                            new_token_ids,
                            engine_core_output.power_smc_logprobs,
                            str(finish_reason)
                    ...
                    if (
                        should_check_power_smc_resample
                        and req_state.parent_req not in power_smc_parent_req_set
                    ):
                        power_smc_parent_reqs.append(req_state.parent_req)
                        power_smc_parent_req_set.add(req_state.parent_req)
                    if engine_core_output.power_smc_kv_event is not None:
                        power_smc_kv_events.append((
                            req_state.parent_req,
                            engine_core_output.power_smc_kv_event,
                        ))
        ...
        for parent_req in power_smc_parent_reqs:
            parent_req.maybe_resample_power_smc()
        for parent_req, kv_event in power_smc_kv_events:
            parent_req.observe_power_smc_kv_event(kv_event)

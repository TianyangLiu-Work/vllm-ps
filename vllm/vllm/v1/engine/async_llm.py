...
from vllm.v1.engine.parallel_sampling import ParentRequest, PowerSMCParentRequest
    ...
    async def add_request(
        ...
        if isinstance(prompt, AsyncGenerator):
            ...
            validate_power_smc_engine_features(
                ...
                kv_block_size=self.vllm_config.cache_config.block_size,
            )
        ...
        if is_pooling or (power_smc_config is None and n == 1):
        ...
        # Fan out child requests (for n>1 or Power-SMC particles).
        parent_request = (
            PowerSMCParentRequest(request, power_smc_config)
            if power_smc_config is not None
            else ParentRequest(request)
        )
        for idx in range(n):

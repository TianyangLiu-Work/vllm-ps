...
class KVCacheManager:
    ...
    def alias_request_blocks(
        ...
        """Alias full KV block prefixes between requests.

        This updates the scheduler-owned block tables and BlockPool refcounts.
        It intentionally does not mutate Request token state or worker-side
        input batch rows; scheduler callers must do that before rescheduling.
        """
        self.coordinator.alias_request_blocks(
            dst_request_id,
            src_request_id,
            num_prefix_blocks,
        )
    ...
    def alias_request_blocks_from_snapshot(
        self,
        dst_request_id: str,
        src_blocks: KVCacheBlocks,
        src_num_cached_blocks: list[int],
        num_prefix_blocks: int,
    ) -> None:
        """Alias full KV block prefixes from a pre-resampling snapshot."""
        self.coordinator.alias_request_blocks_from_snapshot(
            dst_request_id,
            src_blocks.blocks,
    ...
    def get_num_cached_blocks(self, request_id: str) -> list[int]:
        return self.coordinator.get_num_cached_blocks(request_id)

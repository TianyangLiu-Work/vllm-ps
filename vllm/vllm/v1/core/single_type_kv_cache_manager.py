...
class SingleTypeKVCacheManager(ABC):
    ...
    def alias_request_blocks(
        self,
        dst_request_id: str,
        src_request_id: str,
        num_prefix_blocks: int,
    ) -> None:
        """Alias a request's block prefix to another request.

        This is a low-level ownership primitive for block-boundary resampling.
        It updates scheduler-side block ownership and refcounts only; callers
        are responsible for aligning request token state and worker block table
        rows before the next forward pass.
        """
        src_blocks = self.req_to_blocks.get(src_request_id)
        if src_blocks is None:
            raise ValueError(f"Source request {src_request_id!r} has no blocks.")
        self.alias_request_blocks_from_snapshot(
            dst_request_id=dst_request_id,
            src_blocks=list(src_blocks),
            src_num_cached_blocks=self.num_cached_block.get(src_request_id, 0),
    ...
    def alias_request_blocks_from_snapshot(
        self,
        dst_request_id: str,
        src_blocks: Sequence[KVCacheBlock],
        src_num_cached_blocks: int,
        num_prefix_blocks: int,
    ) -> None:
        """Alias a request prefix from a previously captured block snapshot."""
        if num_prefix_blocks < 0:
            raise ValueError("num_prefix_blocks must be non-negative.")
        if len(src_blocks) < num_prefix_blocks:
            raise ValueError(
                f"Source block snapshot has fewer than {num_prefix_blocks} "
                "blocks.")

        old_dst_blocks = self.req_to_blocks.pop(dst_request_id, [])
        if old_dst_blocks:
            self.block_pool.free_blocks(reversed(old_dst_blocks))

        aliased_blocks = list(src_blocks[:num_prefix_blocks])
        self.req_to_blocks[dst_request_id] = aliased_blocks
        self.block_pool.touch(
            [block for block in aliased_blocks if not block.is_null])
        self.num_cached_block[dst_request_id] = min(src_num_cached_blocks,
                                                    num_prefix_blocks)

...
class KVCacheCoordinator(ABC):
    ...
    def alias_request_blocks(
        ...
        """Alias a request's block prefix to another request in every group."""
        for manager in self.single_type_managers:
    ...
    def alias_request_blocks_from_snapshot(
        self,
        dst_request_id: str,
        src_blocks: tuple[Sequence[KVCacheBlock], ...],
        src_num_cached_blocks: list[int],
        num_prefix_blocks: int,
    ) -> None:
        """Alias a request's block prefix from a pre-resampling snapshot."""
        if len(src_blocks) != len(self.single_type_managers):
            raise ValueError("src_blocks must have one entry per KV cache group.")
        if len(src_num_cached_blocks) != len(self.single_type_managers):
            raise ValueError(
                "src_num_cached_blocks must have one entry per KV cache group.")
        for i, manager in enumerate(self.single_type_managers):
            manager.alias_request_blocks_from_snapshot(
                dst_request_id=dst_request_id,
                src_blocks=src_blocks[i],
                src_num_cached_blocks=src_num_cached_blocks[i],

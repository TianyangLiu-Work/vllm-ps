...
from vllm.v1.power_smc import PowerSMCGroupManager
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Scheduler-side Power-SMC state. The output processor owns the
        # user-facing aggregation; the scheduler owns request/KV mutation.
        self.power_smc_group_managers: dict[str, PowerSMCGroupManager] = {}
        self.power_smc_group_children: dict[str, dict[int, str]] = {}
        self._last_power_smc_kv_event: dict[str, Any] | None = None
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            if (
                request.power_smc_config is not None
                and self._power_smc_should_pause_at_group_boundary(request)
            ):
                req_index += 1
                continue
    ...
    def update_from_output(
        ...
        power_smc_logprobs = model_runner_output.power_smc_logprobs
        ...
        resampled_reqs: list[Request] = []
        power_smc_parents_to_resample: list[str] = []
        power_smc_output_by_parent: dict[str, EngineCoreOutput] = {}
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            new_power_smc_logprobs = None
            new_power_smc_kv_event = None
            power_smc_parent_id = None
            if request.power_smc_config is not None and new_token_ids:
                if power_smc_logprobs is not None:
                    new_power_smc_logprobs = power_smc_logprobs.slice_request(req_index)
                if not stopped:
                    power_smc_parent_id = self._observe_power_smc_request_output(
                        request,
                        new_token_ids,
                        new_power_smc_logprobs,
                        finish_reason,
                        request.stop_reason,
                    )
                    if (
                        power_smc_parent_id is not None
                        and power_smc_parent_id not in power_smc_parents_to_resample
                    ):
                        power_smc_parents_to_resample.append(power_smc_parent_id)
            ...
            if (
                ...
                engine_core_output = EngineCoreOutput(
                    ...
                    power_smc_logprobs=new_power_smc_logprobs,
                    power_smc_kv_event=new_power_smc_kv_event,
                ...
                if power_smc_parent_id is not None:
                    power_smc_output_by_parent[power_smc_parent_id] = (
                        engine_core_output)
            else:
                # Invariant: EngineCore returns no partial prefill outputs.
                assert not prompt_logprobs_tensors

        for parent_id in power_smc_parents_to_resample:
            self._last_power_smc_kv_event = None
            resampled_reqs.extend(
                self._maybe_resample_power_smc_parent(parent_id))
            power_smc_kv_event = self._last_power_smc_kv_event
            self._last_power_smc_kv_event = None
            if power_smc_kv_event is not None:
                power_smc_output = power_smc_output_by_parent.get(parent_id)
                if power_smc_output is not None:
                    power_smc_output.power_smc_kv_event = power_smc_kv_event
        ...
        if resampled_reqs:
            resampled_set = set(resampled_reqs)
            self.running = remove_all(self.running, resampled_set)
            self.waiting.remove_requests(resampled_set)
            self.skipped_waiting.remove_requests(resampled_set)
            for request in reversed(resampled_reqs):
    ...
    def _power_smc_should_pause_at_group_boundary(self, request: Request) -> bool:
        """Hold fast particles at a block boundary until siblings catch up."""

        config = request.power_smc_config
        if config is None:
            return False
        child_info = request.power_smc_child_info
        if child_info is None:
            return False

        # Do not block replay/recompute work after a resampling reset. Boundary
        # synchronization only applies when the request is ready to decode a new
        # token beyond its current logical sequence.
        if request.num_computed_tokens < request.num_tokens:
            return False

        output_length = request.num_output_tokens
        if output_length == 0 or output_length % config.block_size != 0:
            return False

        manager = self.power_smc_group_managers.get(child_info.parent_id)
        if manager is None:
            return False
        return any(
            len(particle.token_ids) < output_length
            for particle in manager.particles
        )

    def _get_power_smc_group_manager(
        self,
        request: Request,
    ) -> tuple[str, int, PowerSMCGroupManager] | None:
        if request.power_smc_config is None:
            return None
        child_info = request.power_smc_child_info
        if child_info is None:
            return None
        if child_info.particle_idx >= request.power_smc_config.particles:
            raise RuntimeError(
                "Power-SMC child particle index exceeds configured particles: "
        ...
        if manager is None:
            seed = (
                ...
                or request.sampling_params.seed is None
                else request.sampling_params.seed - child_info.particle_idx
            )
            manager = PowerSMCGroupManager(
                request.power_smc_config,
                random.Random(seed),
            )
            self.power_smc_group_managers[child_info.parent_id] = manager
            self.power_smc_group_children[child_info.parent_id] = {}

        self.power_smc_group_children[child_info.parent_id][
            child_info.particle_idx
        ] = request.request_id
        return child_info.parent_id, child_info.particle_idx, manager

    def _observe_power_smc_request_output(
        self,
        request: Request,
        ...
        if not new_token_ids:
            return None
        group_info = self._get_power_smc_group_manager(request)
        ...
        if len(new_token_ids) != 1:
            raise RuntimeError(
                ...
                f"decode step, got {len(new_token_ids)}."
            )

        parent_id, particle_idx, manager = group_info
        base_logp, proposal_logq = power_smc_logprobs
        is_block_boundary = manager.update_after_token(
            particle_idx,
            new_token_ids[0],
            base_logp=base_logp,
    ...
    def _maybe_resample_power_smc_parent(
        self,
        parent_id: str,
    ) -> list[Request]:
        manager = self.power_smc_group_managers.get(parent_id)
        if manager is None:
            return []
        if not manager.maybe_resample():
            return []
        return self._apply_power_smc_resample_plan(parent_id, manager)

    def _apply_power_smc_resample_plan(
        self,
        parent_id: str,
        manager: PowerSMCGroupManager,
    ) -> list[Request]:
        plan = manager.last_resample_plan
        if plan is None:
            return []

        child_request_ids = self.power_smc_group_children.get(parent_id, {})
        kv_alias_supported = self._power_smc_kv_alias_supported()
        can_alias_kv = manager.config.kv_cow and kv_alias_supported
        kv_event: dict[str, Any] = {
            "resample_index": manager.resample_count,
            "kv_cow_enabled": manager.config.kv_cow,
            "kv_alias_supported": kv_alias_supported,
            "kv_mode": (
                "snapshot_alias_replay" if can_alias_kv else
                "disabled_reset_recompute" if not manager.config.kv_cow else
                "unsupported_reset_recompute"),
        ...
        if manager.config.kv_pool_diagnostics:
            self._add_power_smc_kv_pool_snapshot(kv_event, "before")

        ancestor_snapshots = {}
        needed_ancestor_request_ids: dict[int, str] = {}
        aliased_blocks_by_ancestor: dict[int, int] = {}
        identity_noop_particles = [False] * len(plan.particle_token_ids)
        block_size = int(kv_event["block_size"])
        ancestors = plan.ancestors
        requests = self.requests
        kv_cache_manager = self.kv_cache_manager
        encoder_cache_manager = self.encoder_cache_manager
        inflight_prefills = self._inflight_prefills
        all_children_scheduler_noop = True
        active_child_entries: list[tuple[int, list[int], str, Request]] = []
        for particle_idx, token_ids in enumerate(plan.particle_token_ids):
            req_id = child_request_ids.get(particle_idx)
            if req_id is None:
                all_children_scheduler_noop = False
                continue
            request = requests.get(req_id)
            if request is None or request.is_finished():
                all_children_scheduler_noop = False
                continue
            active_child_entries.append((
                particle_idx,
                token_ids,
                req_id,
                request,
            ))
            ancestor_idx = ancestors[particle_idx]
            if self._power_smc_child_resample_is_scheduler_noop(
                request,
                token_ids,
                ancestor_idx == particle_idx,
            ):
                identity_noop_particles[particle_idx] = True
                continue

            all_children_scheduler_noop = False
            if can_alias_kv:
                ancestor_req_id = child_request_ids.get(ancestor_idx)
                if ancestor_req_id is not None and ancestor_req_id in requests:
                    needed_ancestor_request_ids.setdefault(
                        ancestor_idx,
                        ancestor_req_id,
                    )

        if all_children_scheduler_noop:
            kv_event["kv_mode"] = "identity_noop"
            kv_event["child_count"] = len(plan.particle_token_ids)
            kv_event["identity_noop_count"] = len(plan.particle_token_ids)
            if manager.config.kv_pool_diagnostics:
                self._add_power_smc_kv_pool_snapshot(kv_event, "after")
            manager.record_kv_resample_event(kv_event)
            self._last_power_smc_kv_event = kv_event
            return []

        if can_alias_kv:
            for ancestor_idx, req_id in needed_ancestor_request_ids.items():
                ancestor_snapshots[ancestor_idx] = (
                    kv_cache_manager.get_blocks(req_id),
                    kv_cache_manager.get_num_cached_blocks(req_id),
                )
            kv_event["snapshot_count"] = len(ancestor_snapshots)

        resampled: list[Request] = []
        child_count = 0
        alias_attempt_count = 0
        alias_success_count = 0
        fallback_count = 0
        ...
        replay_tokens = 0
        identity_noop_count = 0
        for particle_idx, token_ids, req_id, request in active_child_entries:
            child_count += 1
            aliased_computed_tokens = 0
            ancestor_idx = ancestors[particle_idx]
            if identity_noop_particles[particle_idx]:
                identity_noop_count += 1
                continue
            if (
                can_alias_kv
                and ancestor_idx in ancestor_snapshots
            ):
                src_blocks, src_num_cached_blocks = ancestor_snapshots[ancestor_idx]
                # Replay at least the last token after resampling so the next
                # forward pass does not overwrite the final shared KV block.
                replay_safe_tokens = (
                    request.num_prompt_tokens + len(token_ids) - 1
                )
                num_alias_blocks = (
                    0 if replay_safe_tokens <= 0 else
                    replay_safe_tokens // block_size)
                if num_alias_blocks > 0:
                    alias_attempt_count += 1
                    try:
                        kv_cache_manager.alias_request_blocks_from_snapshot(
                            dst_request_id=req_id,
                            src_blocks=src_blocks,
                            src_num_cached_blocks=src_num_cached_blocks,
                            num_prefix_blocks=num_alias_blocks,
                        )
                        aliased_computed_tokens = (
                            num_alias_blocks * block_size)
                        alias_success_count += 1
                        aliased_blocks += num_alias_blocks
                        aliased_tokens += aliased_computed_tokens
                        aliased_blocks_by_ancestor[ancestor_idx] = max(
                            aliased_blocks_by_ancestor.get(ancestor_idx, 0),
                            num_alias_blocks,
                        )
                    except ValueError:
                        aliased_computed_tokens = 0

            if aliased_computed_tokens == 0:
                fallback_count += 1
        ...
        if resampled:
            ...
            kv_event["fallback_count"] = fallback_count
            kv_event["aliased_blocks"] = aliased_blocks
            kv_event["aliased_tokens"] = aliased_tokens
            kv_event["replay_tokens"] = replay_tokens
            kv_event["identity_noop_count"] = identity_noop_count
            cow_physical_blocks = sum(aliased_blocks_by_ancestor.values())
            kv_event["cow_physical_blocks"] = cow_physical_blocks
            kv_event["cow_saved_blocks"] = max(
                aliased_blocks - cow_physical_blocks, 0)
            kv_event["cow_saved_tokens"] = (
                int(kv_event["cow_saved_blocks"])
                * block_size)
            if manager.config.kv_pool_diagnostics:
                self._add_power_smc_kv_pool_snapshot(kv_event, "after")
            manager.record_kv_resample_event(kv_event)
    ...
    def _power_smc_child_resample_is_scheduler_noop(
        self,
        request: Request,
        token_ids: list[int],
        same_ancestor: bool,
        ...
        if not same_ancestor:
            return False
        if not request.has_output_token_ids(token_ids):
            return False
        if request.num_computed_tokens < request.num_tokens:
            return False
        return not (
    ...
    def _add_power_smc_kv_pool_snapshot(
        self,
        kv_event: dict[str, Any],
        suffix: str,
    ) -> None:
        kv_event.update({
            f"kv_pool_{key}_{suffix}": value
            for key, value in self._power_smc_kv_pool_snapshot().items()
        })

    def _power_smc_kv_pool_snapshot(self) -> dict[str, int | float]:
        block_pool = getattr(self.kv_cache_manager, "block_pool", None)
        if block_pool is None:
            return {}
        total_blocks = getattr(block_pool, "num_gpu_blocks", None)
        get_num_free_blocks = getattr(block_pool, "get_num_free_blocks", None)
        if not isinstance(total_blocks, int) or get_num_free_blocks is None:
            return {}
        free_blocks = int(get_num_free_blocks())
        snapshot: dict[str, int | float] = {
            "total_blocks": total_blocks,
            "free_blocks": free_blocks,
            "used_blocks": total_blocks - free_blocks,
        }
        get_usage = getattr(block_pool, "get_usage", None)
        if get_usage is not None:
            snapshot["usage"] = float(get_usage())
        return snapshot

    def _power_smc_kv_alias_supported(self) -> bool:
    ...
    def _free_blocks(self, request: Request):
        ...
        if child_info is not None:
            children = self.power_smc_group_children.get(child_info.parent_id)
            if children is not None:
                children.pop(child_info.particle_idx, None)
                if not children:
                    self.power_smc_group_children.pop(child_info.parent_id, None)
                    self.power_smc_group_managers.pop(child_info.parent_id, None)

...
from verl.workers.rollout.vllm_rollout.utils import (
    ...
    apply_power_smc_sampling_params,
    build_power_smc_extra_args,
...
class TestPowerSMCSamplingParams:
    def test_disabled_does_not_inject_extra_args(self):
        sampling_params = {"temperature": 1.0}

        result = apply_power_smc_sampling_params(
            sampling_params,
            {"enabled": False, "alpha": 1.4},
        )

        assert result is sampling_params
        assert "extra_args" not in result

    def test_alpha_one_keeps_plain_vllm_fast_path(self):
        sampling_params = {"temperature": 1.0}

        result = apply_power_smc_sampling_params(
            sampling_params,
            {"enabled": True, "alpha": 1.0, "particles": 8},
        )

        assert result is sampling_params
        assert "extra_args" not in result

    def test_enabled_config_injects_power_smc_extra_args(self):
        result = build_power_smc_extra_args(
            SimpleNamespace(
                enabled=True,
                alpha=1.4,
                particles=8,
    ...
    def test_existing_extra_args_are_preserved(self):
        result = apply_power_smc_sampling_params(
            {"temperature": 1.0, "extra_args": {"trace_id": "abc"}},
            {"enabled": True, "alpha": 1.2, "particles": 4},
        )

        assert result["extra_args"]["trace_id"] == "abc"
        assert result["extra_args"]["power_smc"]["particles"] == 4

    def test_particles_must_be_positive(self):
        with pytest.raises(ValueError, match="particles must be >= 1"):
            build_power_smc_extra_args(
                {"enabled": True, "alpha": 1.2, "particles": 0}
            )

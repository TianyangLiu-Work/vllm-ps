...
def build_power_smc_extra_args(power_smc_config: Any) -> dict[str, Any] | None:
    """Build vLLM SamplingParams.extra_args for Power-SMC.

    Returns None when disabled or when alpha=1.0, which intentionally keeps the
    request on the plain vLLM batch-sampling path.
    """

    if not _config_get(power_smc_config, "enabled", False):
        return None

    alpha = float(_config_get(power_smc_config, "alpha", 1.4))
    particles = int(_config_get(power_smc_config, "particles", 4))
    block_size = int(_config_get(power_smc_config, "block_size", 64))
    ess_threshold = float(_config_get(power_smc_config, "ess_threshold", 0.5))
    alpha_ramp_tokens = int(_config_get(power_smc_config, "alpha_ramp_tokens", 400))
    proposal = _config_get(power_smc_config, "proposal", "power_temperature")

    if alpha <= 0.0:
        raise ValueError(f"power_smc.alpha must be > 0, got {alpha}")
    if particles < 1:
        raise ValueError(f"power_smc.particles must be >= 1, got {particles}")
    if block_size < 1:
        raise ValueError(f"power_smc.block_size must be >= 1, got {block_size}")
    if not 0.0 < ess_threshold <= 1.0:
        raise ValueError(
            "power_smc.ess_threshold must be in (0, 1], "
            f"got {ess_threshold}"
        )
    if alpha_ramp_tokens < 1:
        raise ValueError(
            "power_smc.alpha_ramp_tokens must be >= 1, "
            f"got {alpha_ramp_tokens}"
        )
    if proposal != "power_temperature":
        raise ValueError(
            "power_smc.proposal currently supports only "
            f"'power_temperature', got {proposal!r}"
        )
    ...
    return {
        "power_smc": {
            ...
            "return_diagnostics": bool(
                _config_get(power_smc_config, "return_diagnostics", False)
            ),
            "kv_cow": bool(_config_get(power_smc_config, "kv_cow", True)),
            "kv_pool_diagnostics": bool(
                _config_get(power_smc_config, "kv_pool_diagnostics", False)
            ),
        }
    }


def apply_power_smc_sampling_params(
    sampling_params: dict[str, Any],
    power_smc_config: Any,
) -> dict[str, Any]:
    """Inject Power-SMC extra_args into a vLLM sampling params dict."""

    power_smc_extra_args = build_power_smc_extra_args(power_smc_config)
    if power_smc_extra_args is None:
        return sampling_params

    sampling_params = dict(sampling_params)
    extra_args = dict(sampling_params.get("extra_args") or {})
    extra_args.update(power_smc_extra_args)

...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        sampling_params = apply_power_smc_sampling_params(sampling_params, self.config.power_smc)
        ...
        if sampling_params.logprobs is not None:
            output_logprobs = final_res.outputs[0].logprobs or []
            log_probs = []
            for i, token_id in enumerate(token_ids):
                if i >= len(output_logprobs):
                    log_probs.append(0.0)
                    continue
                logprobs = output_logprobs[i] or {}
                if token_id in logprobs:
                    log_probs.append(logprobs[token_id].logprob)
                elif logprobs:
                    # Power-SMC may rewrite the final token stream after vLLM
                    # has already built per-step logprob maps for child outputs.
                    # Keep the adapter robust and preserve shape for downstream
                    # recomputation paths.
                    log_probs.append(next(iter(logprobs.values())).logprob)
                else:
                    log_probs.append(0.0)

...
@dataclass
class PowerSMCRolloutConfig(BaseConfig):
    """Power-SMC options forwarded to a vLLM fork that supports extra_args."""
    ...
    proposal: str = "power_temperature"
    ...
    def __post_init__(self):
        if self.alpha <= 0.0:
            raise ValueError(f"power_smc.alpha must be > 0, got {self.alpha}")
        if self.particles < 1:
            raise ValueError(f"power_smc.particles must be >= 1, got {self.particles}")
        if self.block_size < 1:
            raise ValueError(f"power_smc.block_size must be >= 1, got {self.block_size}")
...
class RolloutConfig(BaseConfig):
    ...
    power_smc: PowerSMCRolloutConfig = field(default_factory=PowerSMCRolloutConfig)
